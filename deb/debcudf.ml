...
let init_tables ?(compare=Version.compare) pkglist =
  ...
  let l = Hashtbl.fold (fun v p acc -> (p,v)::acc) temp_versions_table [] in
  let cmp (x,_) (y,_) = Version.compare x y in
  ...
  let rec numbers (prec,i) = function
    ...
    |(p,v)::t ->
      if (Version.compare v prec) = 0 then begin
        ...
        Hashtbl.add tables.reverse_table (p,i) v ;
        numbers (prec,i) t
      end else begin
        Hashtbl.add tables.versions_table v (i+1);
        (* this one is multi binding *)
        Hashtbl.add tables.reverse_table (p,(i+1)) v ;
        numbers (v,(i+1)) t
      end
  in
