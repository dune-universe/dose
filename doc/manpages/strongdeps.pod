...
B<strongdeps> prints on standard output in the YAML format the list of the
packages that strong depends on each of the packages specified. If
B<--checkonly> if coupled with B<--dot> then B<strongdeps> prints on standard
output in dot format the detransitivitazed graph of the strong dependencies of
each package.

Since B<--checkonly> computes only the strong dependencies of a set of
packages, the impact set with relation to the entire universe is not computed.
