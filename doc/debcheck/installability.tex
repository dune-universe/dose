...
\begin{itemize}
\item The set complete $R$ since it is not flat (it contains two
...
\begin{example}
  In the above example, $(a,1)$ is installable, while $(a,2)$ is not.
\end{example}
...
\item Installability means theoretical existence of a solution. It
  ...
  \texttt{apt-get}) actually finds a way to the install that package.
...
be installed \emph{together}. See Section~\ref{sec:tricks}.
