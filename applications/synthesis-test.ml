...
begin
  ...
  Cudf.iter_packages (fun sp ->
    ...
    List.iter (fun (n, v) ->
      if n.[0] = '/' then
      begin
        let s_provides = s_map.CudfAdd.who_provides ((n, v) :> Cudf_types.vpkg) in
        let h_provides = h_map.CudfAdd.who_provides ((n, v) :> Cudf_types.vpkg) in
        let slist = List.map (fun pn -> string_of_pkgname pn.package) (List.unique (List.sort s_provides)) in
        let hlist = List.map (fun pn -> string_of_pkgname pn.package) (List.unique (List.sort h_provides)) in
        if List.length slist <> List.length hlist
        then
          Printf.printf "ERROR: for package %s-%s, file dependency on %s:\nsynthesis says: %s\nhdlist says:%s\n"
          (string_of_pkgname sp.package) (string_of_version sp.version) (string_of_pkgname n) (String.concat ", " slist) (String.concat ", " hlist)
        else if not (List.fold_left2 (fun acc s h -> acc && s = h) true slist hlist)
        then
          Printf.printf "ERROR: for package %s-%s, file dependency on %s:\nsynthesis says: %s\nhdlist says:%s\n"
          (string_of_pkgname sp.package) (string_of_version sp.version) (string_of_pkgname n) (String.concat ", " slist) (String.concat ", " hlist)
      end
